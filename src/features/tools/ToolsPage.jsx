...
export default function ToolsPage() {
  ...
  return (
    <div className="flex-1 bg-slate-50/50 overflow-y-auto select-none selection:bg-indigo-100 font-sans">
      {/* Page Header */}
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 p-5 rounded-2xl bg-white border border-gray-200 shadow-xs sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-gray-800">Application Connections</h3>
                    ...
                    return (
                      <tr key={conn.id} className="hover:bg-slate-50/30 transition-colors">
                        <td className="px-6 py-4.5 font-bold text-gray-900 flex items-center gap-3">
                          <div
                            className={`h-8 w-8 rounded-lg flex items-center justify-center ${conn.iconColor} shadow-3xs border border-gray-200/40 text-xs`}
                          >
                            {conn.name?.charAt(0)}
                          </div>
                          <div>
                            ...
                            {conn.count > 0 && (
                              <span className="font-medium text-gray-400 text-[11px] ml-1.5">
                                ({conn.count} tools)
                              </span>
                            )}
                          </div>
                        ...
                        <td className="px-6 py-4.5">
                          <span
                            className={`text-[9px] font-extrabold tracking-wider uppercase px-2 py-0.5 rounded-full ${
                              conn.type === 'MCP'
                                ? 'bg-purple-100/60 text-purple-600 border border-purple-200/40'
                        ...
                        <td className="px-6 py-4.5 font-mono text-[11px] text-gray-600">
                          {savedKeyDisplay ? (
                            <span className="bg-slate-100 px-2 py-0.5 rounded border border-slate-200 font-bold text-indigo-600">
                              {maskSecret(savedKeyDisplay)}
                            </span>
                          ) : (
                            <span className="text-gray-400 italic">No Key Saved</span>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4.5 text-center flex items-center justify-center gap-2">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-[11px] font-bold bg-indigo-50 text-indigo-600 border border-indigo-200 hover:bg-indigo-100 transition"
                          >
          ...
          <div className="space-y-6">
            <div className="flex flex-col gap-4 p-5 rounded-2xl bg-white border border-gray-200 shadow-xs sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button
                variant="secondary"
                size="sm"
                className="whitespace-nowrap font-semibold gap-1.5 border-gray-250 hover:bg-gray-50 text-gray-700 bg-white"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredInternalTools.map((tool) => (
                <div
                  key={tool.id}
                  className="border border-gray-200 bg-white rounded-2xl p-5 shadow-3xs flex flex-col justify-between min-h-[120px]"
                >
                  <div>
                  ...
                  <div className="text-[10px] text-gray-400 mt-3 pt-3 border-t border-gray-50 flex items-center justify-between">
                    <span>
                      ID:{' '}
                      <code className="font-mono text-[9.5px] bg-slate-50 px-1 py-0.5 rounded border border-gray-100">
                        {tool.id}
                      </code>
                    </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {defaultIntegrations.map((integ) => {
                ...
                return (
                  <div
                    key={integ.id}
                    className="border border-gray-200 bg-white rounded-2xl p-5 shadow-3xs flex justify-between gap-4"
                  >
                    <div className="space-y-1 flex-1">
                      ...
                      {savedKey && (
                        <div className="pt-2 text-[11px] font-mono text-gray-500">
                          Saved Key: <span className="bg-slate-100 px-1.5 py-0.5 rounded font-bold text-indigo-600">{maskSecret(savedKey)}</span>
                        </div>
                      )}
                    </div>
      ...
      {selectedToolConfig && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-xs p-4">
          <div className="bg-white border border-gray-200 rounded-3xl max-w-lg w-full p-6 shadow-xl space-y-5 animate-in fade-in zoom-in duration-150">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-500">Input your real API tokens or PAT for runtime execution</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSaveConfig} className="space-y-4">
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full text-xs bg-gray-50 border border-gray-250 rounded-xl px-3 py-2.5 outline-none focus:border-indigo-500 focus:bg-white font-mono"
                    />
                    <span className="text-[10px] text-gray-400 mt-1 block">
                      Requires <code className="bg-gray-100 px-1 py-0.5 rounded">repo</code> and <code className="bg-gray-100 px-1 py-0.5 rounded">issues</code> scopes.
                    </span>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Default Repo Owner</label>
                      <input
                        type="text"
                        ...
                        className="w-full text-xs bg-gray-50 border border-gray-250 rounded-xl px-3 py-2 outline-none focus:border-indigo-500 focus:bg-white font-medium"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-gray-700 mb-1">Default Repo Name</label>
                      <input
                        type="text"
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                <button
                  ...
                  disabled={isTesting}
                  className="px-3.5 py-2 text-xs font-bold text-gray-700 border border-gray-250 rounded-xl hover:bg-gray-50 transition"
                >
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setSelectedToolConfig(null)}
                    className="px-3.5 py-2 text-xs font-bold text-gray-500 hover:text-gray-800"
                  >
                    Cancel
                  </button>
                  <button
